...
R__LOAD_LIBRARY(libana_bkg)

using namespace std;

int Fun4Sim_bkg(const int nevent = 10)
{ 
  const bool single_gun =false;
  ...
  const bool do_kmag       = false;
  const bool do_absorber   = false;
...
// multi particle gun
  ...
  if(pythia)  //change the pythia configuration file to change the dimuons generated
   {
    ...
    if (bkg)
    {
      ...
      //hr->Set_PionDecayMomRange(-2.,2.,-2.,2.,5.,100.);
      //hr->Set_PionDecayPosRange(-500.,500.,-500.,500.,-800.,25.);
    }
