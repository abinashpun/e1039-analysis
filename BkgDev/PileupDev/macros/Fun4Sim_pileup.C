...
int Fun4Sim_pileup(const int nevent = 1, const int pileupN = 5000)
{
...
// multi particle gun
  ...
  if(pileup){
    ...
    extgen->setExtInputFile("bkg_5M.root");
    extgen->setBucketSize(pileupN);
...
*/
 ...
 se->registerSubsystem(vertexing);
  ...
  ana_pileup->set_out_name(Form("ana_pileup_%d.root",pileupN));
